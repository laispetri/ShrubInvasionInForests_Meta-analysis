...
## mechanisms
x<-as.data.frame(inlmisc::GetColors(3, scheme = "bright"))
BRcolor <- x[1,]
ENcolor <- x[2,]
PPcolor <- x[3,]

## abiotic factors
x<-as.data.frame(inlmisc::GetColors(3, scheme = "high-contrast"))
Lightcolor <- x[1,]
Nutrientcolor <- x[2,]
Watercolor <- x[3,]

## number of invaders
x<-as.data.frame(inlmisc::GetColors(2, scheme = "vibrant"))
Singlecolor <- x[1,]
Multicolor <- x[2,]

## type of ES
x<-as.data.frame(inlmisc::GetColors(2, scheme = "muted"))
ESNatcolor <- x[1,]
ESInvcolor <- x[2,]
```
...
# parameters
parameters <- c("alpha", "beta", "invESavemean.h", "invpred", "tau", "var")
...
# parameters
parameters <- c("beta", "natESavemean.h", "natpred", "tau", "var")
...
forest <- forest %>% 
  select(mean, sd, `2.5%`, `97.5%`) %>% 
  rownames_to_column(., var = "rowname") %>% 
  filter(!grepl("^natESavemean.h", rowname),!grepl("^B.h", rowname), rowname != "deviance", !grepl("^alpha", rowname), !grepl("^natpred", rowname),!grepl("^tau", rowname),!grepl("^var", rowname)) %>%
  arrange(rowname) %>%
