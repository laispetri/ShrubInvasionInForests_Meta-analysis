...
InvEstimateFP %>% 
  ...
  lm(y ~ x, data = .) %>% 
  tidy(conf.int = TRUE) #funnel plot is asymmetrical, intercept is statistically significant

NatBioticEstimateFP %>% 
  mutate(y = ESNatmean/ESNatSD, x = Variance) %>% 
  lm(y ~ x, data = .) %>% 
  tidy(conf.int = TRUE) #funnel plot is not asymmetrical, intercept contains zero
